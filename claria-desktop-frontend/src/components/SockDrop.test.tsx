...
describe("SockDrop", () => {
  it("runs drop → walk-in → grab → shake → walk-off, then clears", () => {
    ...
    // The moment she grabs it, the sock moves from the floor to her mouth.
...
describe("pixel sprites", () => {
  ...
  it("dog frames differ only in the legs", () => {
    ...
    expect(DOG_FRAME_A.slice(0, 12)).toEqual(DOG_FRAME_B.slice(0, 12));
    expect(DOG_FRAME_A.slice(12)).not.toEqual(DOG_FRAME_B.slice(12));
  });
});
