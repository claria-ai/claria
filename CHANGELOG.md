...
### Added
- **CloudTrail activity viewer** — new "Activity" page reachable from the Infrastructure chat toolbar. Lists recent AWS API calls against the account via `cloudtrail:LookupEvents` (free, last 90 days), with filters for time window (1h–90d), event name, and read-only vs write. Each row opens a detail modal with the full event payload. Adds `cloudtrail:LookupEvents` to the scoped IAM user's inline policy.
