...
### Added
- **Multi-lingual + Medical transcription API** — `claria-transcribe` now accepts `TranscribeOptions { language, speakers, engine, custom_vocabulary }` and returns a structured `TranscriptResult` (segments with speaker/timestamp/language, speaker list). Supports Spanish (`es-US`), code-switching English/Spanish via `IdentifyMultipleLanguages`, Transcribe Medical (English-only, PHI tagging via `ContentIdentificationType=PHI`), speaker diarization (1/2/3-4 speakers), and channel identification for stereo audio. Engine is auto-routed: Medical falls back to Standard for non-English languages. New `format_transcript_body`/`parse_transcript_body` render and parse a `[Speaker mm:ss\u{2013}mm:ss lang]`-headered plain-text format so the existing `.text` sidecar carries structure through user edits; legacy header-less sidecars degrade to a single un-diarized segment. Legacy `transcribe_audio` retained as a thin wrapper during the transition window.
- **`SyncedPreferences` cross-machine sync (Rust scaffolding)** — split `ClariaConfig` into machine-local (region, credentials, account_id, system_name, created_at) and synced (preferred_model_id, cost_explorer_enabled, hourly_cost_data, prompt_caching_enabled, transcription) subsets. The synced subset is destined for `_state/preferences.json` in S3; Tauri command wiring follows in a subsequent commit. New `claria_core::s3_keys::PREFERENCES` constant. Config schema bumped to v6 with a v5→v6 migration that injects `transcription` defaults (English, 2 speakers, Standard engine, no medical, no custom vocab).
- **`get_first_version` S3 helper** — convenience in `claria-storage` that returns the oldest non-delete-marker version of an object. Backs the planned "Reset to original" transcript feature: S3 bucket versioning (already enabled by the provisioner) gives us v1-as-canonical history for free, with no need for a second sidecar file.
